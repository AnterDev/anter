...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("anterMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("anterMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("anterMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("anter-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running anterMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
